...
var _ = Describe("Ironic controller", func() {
	...
	When("Ironic is created with an invalid password", func() {
		BeforeEach(func() {
			DeferCleanup(
				k8sClient.Delete,
				ctx,
				CreateIronicInvalidSecret(ironicNames.Namespace, SecretName),
			)
			DeferCleanup(
				k8sClient.Delete,
				ctx,
	...
	When("Ironic has no inspect interface annotation", func() {
		...
		It("defaults to agent in ironic.conf", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(conf).To(ContainSubstring("default_inspect_interface=agent"))
				g.Expect(conf).To(ContainSubstring("enabled_inspect_interfaces=agent,"))
				g.Expect(conf).NotTo(ContainSubstring("[inspector]"))
			}, timeout, interval).Should(Succeed())
		})
	})
